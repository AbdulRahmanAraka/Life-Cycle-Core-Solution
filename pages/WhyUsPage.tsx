    ...
    <div className="bg-gradient-to-b from-slate-50 to-white min-h-screen">
      ...
      <div className="bg-gradient-to-r from-slate-900 to-slate-800 py-12 sm:py-16 border-b border-slate-700">
        <div className="container mx-auto px-4 lg:px-20">
          <h1 className="text-3xl sm:text-5xl md:text-6xl font-black text-white tracking-tight mb-4">Why Choose LCS</h1>
      ...
      <div className="container mx-auto px-4 lg:px-20 py-12 sm:py-16">
        <div className="mb-10 sm:mb-16 md:mb-20 text-center reveal active">
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-slate-900 mb-4 sm:mb-6 tracking-tight" style={{ lineHeight: '1.4' }}>
            Accelerate Your Digital Transformation,<br/>
            <span className="text-blue-600">Delivered with Purpose</span>
          </h2>
          <p className="text-base sm:text-xl md:text-2xl text-slate-600 max-w-4xl mx-auto leading-relaxed font-medium">
        ...
        <div className="grid lg:grid-cols-3 gap-6 sm:gap-8 mb-10 sm:mb-16">
          {points.map((point, idx) => (
            <div key={idx} className="group reveal active" style={{ animationDelay: `${idx * 0.1}s` }}>
              <div className="bg-white rounded-[2rem] p-6 sm:p-8 shadow-lg border-2 border-slate-200 hover:border-blue-500 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl h-full flex flex-col relative overflow-hidden">
                {/* Icon */}
                <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br from-blue-600 to-blue-700 rounded-2xl flex items-center justify-center text-white text-xl sm:text-2xl mb-6 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 shadow-lg">
                  <i className={`fas ${point.icon}`}></i>
                </div>

                {/* Title */}
                <h3 className="text-xl sm:text-2xl font-black text-slate-900 mb-4 group-hover:text-blue-600 transition-colors duration-300">
                  {point.title}
                </h3>
                ...
                {/* Note */}
                <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-white p-5 sm:p-6 rounded-2xl mt-auto">
                  <p className="text-xs sm:text-sm font-bold leading-relaxed">{point.note}</p>
                </div>
        ...
        <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-[3rem] p-8 sm:p-12 md:p-16 shadow-2xl reveal active relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-blue-600/10 via-transparent to-blue-600/10"></div>
          
          <div className="relative z-10 grid md:grid-cols-2 gap-12 items-center">
            <div>
              <h3 className="text-2xl sm:text-4xl md:text-5xl font-black text-white mb-4 sm:mb-6 tracking-tight">
                Strategic Advantage
              </h3>
              ...
              <Link to="/contact" className="inline-block bg-blue-600 hover:bg-blue-500 text-white px-8 sm:px-10 py-4 sm:py-5 rounded-2xl font-black text-base sm:text-lg transition-all duration-300 hover:scale-105 shadow-2xl">
                Get a Consultation
              </Link>
            </div>
              ...
              <ul className="space-y-6">
                {advantages.map((item, i) => (
                  <li key={i} className="flex items-start space-x-4 group/item">
                    <div className="flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 bg-blue-600/20 rounded-xl flex items-center justify-center group-hover/item:bg-blue-600 transition-colors duration-300">
                      <i className="fas fa-arrow-right text-blue-400 group-hover/item:text-white transition-colors duration-300"></i>
                    </div>
                    <span className="text-base sm:text-lg font-bold text-slate-300 group-hover/item:text-white transition-colors duration-300 pt-1 sm:pt-2">{item}</span>
                  </li>
                ))}
              </ul>
            </div>
