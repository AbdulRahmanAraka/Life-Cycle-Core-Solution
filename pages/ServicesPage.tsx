...
const ServicesPage: React.FC = () => {
  ...
  return (
    <div className="bg-gradient-to-b from-slate-50 via-white to-slate-50 min-h-screen">
      {/* Header Section */}
      <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 py-12 sm:py-16 md:py-20 border-b border-slate-700 relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-blue-600/10 via-transparent to-blue-600/10"></div>
        <div className="container mx-auto px-4 lg:px-20 relative z-10">
          <h1 className="text-3xl sm:text-5xl md:text-6xl font-black text-white tracking-tight mb-4 animate-fade-in-up">Expert Services</h1>
          <p className="text-slate-300 text-base sm:text-lg md:text-xl max-w-3xl animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
      ...
      {/* Hero Section */}
      <div className="container mx-auto px-4 lg:px-20 py-12 sm:py-16 md:py-20">
        <div className="text-center mb-12 sm:mb-16 md:mb-24 reveal active">
          <div className="inline-block bg-blue-100 text-blue-600 px-4 sm:px-6 py-2 rounded-full font-bold text-xs sm:text-sm uppercase tracking-wider mb-4 sm:mb-6">
            Our Solutions
          </div>
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-slate-900 mb-4 sm:mb-6 tracking-tight" style={{ lineHeight: '1.3' }}>
            Driving Efficiency Across the <span className="text-blue-600">Product Lifecycle</span>
          </h2>
          <p className="text-slate-600 text-base sm:text-xl md:text-2xl leading-relaxed font-medium max-w-4xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-8 sm:gap-12 md:gap-16 mb-12 sm:mb-16 md:mb-20">
          {services.map((service, index) => (
            <div key={service.id} className="group reveal active" style={{ animationDelay: `${index * 0.15}s` }}>
              <div className="bg-white rounded-[2.5rem] p-6 sm:p-8 md:p-10 shadow-xl border-2 border-slate-200 hover:border-blue-500 transition-all duration-500 hover:-translate-y-3 hover:shadow-2xl h-full flex flex-col relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex items-start justify-between mb-6">
                    <h3 className="text-xl sm:text-2xl md:text-3xl font-black text-slate-900 group-hover:text-blue-600 transition-colors duration-300 flex-1">
                      {service.title}
                    </h3>
                    <div className="flex-shrink-0 w-12 h-12 sm:w-14 sm:h-14 bg-gradient-to-br from-blue-600 to-blue-700 rounded-2xl flex items-center justify-center text-white text-lg sm:text-xl group-hover:scale-110 group-hover:rotate-12 transition-all duration-500 shadow-lg ml-4">
                    ...
                    <ul className="space-y-4">
                      {service.points.map((point, i) => (
                        <li key={i} className="flex items-start space-x-3 group/item">
                          <div className="flex-shrink-0 w-5 h-5 sm:w-6 sm:h-6 bg-blue-100 rounded-lg flex items-center justify-center mt-0.5 group-hover/item:bg-blue-600 transition-colors duration-300">
                            <i className="fas fa-check text-blue-600 text-xs group-hover/item:text-white transition-colors duration-300"></i>
                          </div>
                          <span className="text-slate-700 font-semibold text-xs sm:text-sm leading-relaxed">{point}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                  ...
                  {/* Outcome Section */}
                  <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-white p-5 sm:p-6 rounded-2xl mt-auto shadow-xl">
                    ...
                    <p className="text-sm sm:text-base font-bold leading-relaxed">{service.outcome}</p>
                  </div>
                </div>
        ...
        {/* Bottom CTA */}
        <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-[3rem] p-8 sm:p-12 md:p-20 text-center text-white shadow-2xl reveal active relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="inline-block bg-blue-600/20 text-blue-400 px-4 sm:px-6 py-2 rounded-full font-bold text-xs sm:text-sm uppercase tracking-wider mb-6 sm:mb-8 border border-blue-500/30">
              Let's Work Together
            </div>
            <h3 className="text-2xl sm:text-3xl md:text-5xl font-black mb-6 sm:mb-8 tracking-tight leading-tight">
              Ready to Transform Your Operations?
            </h3>
            <p className="text-slate-300 text-base sm:text-lg md:text-xl mb-8 sm:mb-12 max-w-2xl mx-auto leading-relaxed">
              Our experts are ready to analyze your current systems and recommend the right solutions at no cost.
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
              <Link to="/contact" className="bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-500 hover:to-blue-600 text-white px-8 sm:px-12 py-4 sm:py-5 rounded-2xl font-black text-base sm:text-xl transition-all duration-300 hover:scale-110 shadow-2xl flex items-center space-x-2">
                <span>Request a Consultation</span>
                <i className="fas fa-arrow-right"></i>
              </Link>
              <Link to="/about" className="bg-white/10 hover:bg-white/20 text-white border-2 border-white/30 hover:border-white px-8 sm:px-12 py-4 sm:py-5 rounded-2xl font-black text-base sm:text-xl transition-all duration-300 backdrop-blur-sm hover:scale-105">
                Learn More About Us
              </Link>
            </div>
          </div>
